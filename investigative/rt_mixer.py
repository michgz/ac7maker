...
'''Applies real-time parameter changes to the CT-X mixer.
...
def change_time_block(tb):
  ...
  if tb == 0:
    # Reverb
    print("TIME_BLOCK = Reverb")
    send(struct.pack('BBB', 0xB0 + CH, 0x5B, 0x40))
...
def process(x):
  ...
  if (x[0]& 0xF0) == 0xB0 and len(x) >= 3:
    ...
    if (control >= 11 and control <= 19) or (control >= 1 and control <= 9):
      ...
      if TIME_BLOCK == 0:
        #Reverb
        if (control >= 11 and control <= 15):
          v = [129,130,131,132,134][control - 11]
        elif control == 1:
          # reverb type
          v = 128
          y = x[2]//4
          ...
          v = 133
          y = x[2]*4096 + random.randint(0,4095)  # dithered
        ...
        if (control >= 11 and control <= 19):
          ...
          v = 97
          y = x[2]*256 + random.randint(0,255)  # dithered
        elif control == 3:
          v = 106
          y = x[2]//16
        elif control == 4:
        ...
        #Equaliser
        if (control >= 11 and control <= 19):
          v = [160,161,162,163,164,165,166,167,135][control - 11]
          div = [16,4,4,8,8,4,16,1,1][control-11]
          ...
          v = [168,169,192,193,194,195,196,197,198][control - 1]
          div = [1,1,1,1,1,1,1,1,8][control-1]
    ...
    if control == 49:  # "L9". Use this to trigger a download
      ...
      if x[2] > 0:
        ...
        try:
          new_download = download_ac7_internal(0, memory=3, category=2)
