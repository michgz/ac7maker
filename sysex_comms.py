...
def upload_ac7(dest_num, data):
  ...
  if dest_num < 294 or dest_num > 343:
    print("Incorrect destination number")
    raise Exception
  pp = dest_num - 294
  ...
  while i < len(data):
    ...
    pkt = make_packet(parameter_set=pp, category=30, memory=1, command=5, length=len_remaining, data = data[i:i+len_remaining])
    #print(pkt)
    os.write(f, pkt)
    wait_for_ack(f)
    i += len_remaining
  ...
  #print("Sending ESS")
  os.write(f, make_packet(parameter_set=pp, category=30, memory=1, command=0xd))
  time.sleep(0.3)
